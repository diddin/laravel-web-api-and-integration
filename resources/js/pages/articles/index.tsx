...
import { type BreadcrumbItem, type Article, type SharedData } from '@/types';
...
interface ArticlePageProps extends SharedData {
    articles: Article[];
}

export default function Articles() {

    const page = usePage<ArticlePageProps>();
    const articles = page.props.articles as Article[];
    
    const { delete: destroy} = useForm();
    const destroyArticle = (e: any, id: number): void => {
        e.preventDefault();
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                ...
                <div className="relative overflow-x-auto">
                    <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                        {articles.map(({id, title, content}: Article) => (
                            <tr key={id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                ...
                                <td className="px-6 py-4">
                                    <form onSubmit={(e) => destroyArticle(e, id)}>
                                    <Link href={route('articles.edit', id)} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Edit</Link>
